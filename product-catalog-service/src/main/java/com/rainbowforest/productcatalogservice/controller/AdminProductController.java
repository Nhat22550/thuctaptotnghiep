...
public class AdminProductController {
    ...
    private ResponseEntity<Product> updateProduct(@PathVariable("id") Long id, @RequestBody Product product, HttpServletRequest request){
    	...
    	if(existingProduct != null && product != null) {
    		try {
    			...
    			existingProduct.setMaxRange(product.getMaxRange());
    			existingProduct.setTopSpeed(product.getTopSpeed());
    			existingProduct.setColor(product.getColor());
    			existingProduct.setMotorPower(product.getMotorPower());
    			existingProduct.setImageUrl(product.getImageUrl());
    			existingProduct.setSegment(product.getSegment());
    			
    			Product updatedProduct = productService.addProduct(existingProduct); // save
    	        return new ResponseEntity<Product>(
    	        		updatedProduct,
    	        		headerGenerator.getHeadersForSuccessPostMethod(request, updatedProduct.getId()),
    	        		HttpStatus.OK);
    		}catch (Exception e) {
				e.printStackTrace();
				return new ResponseEntity<Product>(
