...
public class DbCheck {
    public static void main(String[] args) {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM email_log ORDER BY id DESC LIMIT 5");
            while (rs.next()) {
                System.out.println("LOG_ROW: ID: " + rs.getLong("id") + ", Order: " + rs.getLong("order_id") + 
                                   ", Status: " + rs.getString("status") + ", Error: " + rs.getString("error_message"));
            }
            conn.close();
        } catch (Exception e) {
            e.printStackTrace();
